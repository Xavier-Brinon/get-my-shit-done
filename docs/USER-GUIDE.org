#+title: GMSD User Guide
#+startup: indent
#+options: toc:2 num:nil ^:{}


A detailed reference for workflows, troubleshooting, and configuration. For quick-start setup, see the [[../README.org][README]].
...
You ran ~/gmsd:new-project~ but ~.planning/PROJECT.org~ already exists. This is a safety check. If you want to start over, delete the ~.planning/~ directory first.
...
Run ~/gmsd:discuss-phase [N]~ before planning. Most plan quality issues come from Claude making assumptions that ~CONTEXT.org~ would have prevented. You can also run ~/gmsd:list-phase-assumptions [N]~ to see what Claude intends to do before committing to a plan.
...
Set ~commit_docs: false~ during ~/gmsd:new-project~ or via ~/gmsd:settings~. This automatically adds ~.planning/~ to your ~.gitignore~. Planning artifacts stay local and never touch git.
