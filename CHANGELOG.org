...
All notable changes to GMSD will be documented in this file.

Format follows [[https://keepachangelog.com/en/1.1.0/][Keep a Changelog]].

* [Unreleased] - GMSD Fork

** Added
- Single-file todo system: ~.planning/TODOS.org~ with ~* Active~ / ~* Archive~ sections replaces file-per-todo ~todos/pending/~ and ~todos/done/~
- ~:LOGBOOK:~ drawer for TODO state change tracking with timestamps and optional reasons
- ~todo cancel~ CLI command: marks todo as CANCELLED, moves to Archive with ~:LOGBOOK:~ entry
- ~--reason~ flag on ~todo complete~, ~todo cancel~, and ~todo update~ for annotating state changes
- ~todo migrate~ CLI command: auto-converts old file-per-todo format to ~TODOS.org~
- Worktree-aware branching: ~execute-phase~ detects worktrees and non-main branches, skips branch creation
- ~complete-milestone~ discovers both ~gmsd/phase-*~ and ~worktree-phase-*~ branches
- Recommended workflow section in README: worktrees + phase branching loop
- Org-mode (~.org~) as document format replacing Markdown (~.md~) with YAML frontmatter
- Org property drawer (~:PROPERTIES:~ / ~:END:~) parser replacing YAML ~---~ frontmatter
- Dot-notation for nested objects in property drawers (e.g., ~:tech-stack.added: [val]~)
- ~DOC_EXT~ and ~FILES~ constants centralized in ~core.cjs~ for all file extension references
- MADR 4.0 ADR workflow: ~/gmsd:create-adr~ command, ~templates/adr.org~ template
- ~DECISIONS_DIR~ in ~core.cjs~, ~adr~ scaffold in ~commands.cjs~ with auto-numbering
- ADR integration in discuss-phase (suggests ADR), execute-phase (verifier checks consistency)
- 4 new ADR-related tests (127/127 pass)
- Converted README, CHANGELOG, and USER-GUIDE to org-mode

** Changed
- Todo documentation updated across help, README, USER-GUIDE, state template, and command files to reflect single-file ~TODOS.org~ system
- ~/gmsd:check-todos~ workflow now supports ~--state~ and ~--priority~ filters, cancel action with reason
- All 35 template files converted from ~.md~ to ~.org~ with full org syntax
- All 7 lib files updated to use ~FILES~ / ~DOC_EXT~ constants from ~core.cjs~
- All 7 test files updated: YAML ~---~ → ~:PROPERTIES:~ / ~:END:~, ~.md~ → ~.org~
- All ~/gsd:*~ commands renamed to ~/gmsd:*~
- All ~gsd-*~ agents renamed to ~gmsd-*~ (11 agents, 3 hooks, 1 tools file)
- Directory ~commands/gsd/~ → ~commands/gmsd/~, ~get-shit-done/~ → ~get-my-shit-done/~
- Branch templates ~gsd/phase-*~ → ~gmsd/phase-*~, settings dir =~/.gsd/= → =~/.gmsd/=
- Data field ~gsd_state_version~ → ~gmsd_state_version~
- Template path references updated across all workflow, agent, and command files
- Generated file references updated: ~PROJECT.org~, ~STATE.org~, ~ROADMAP.org~, etc.

* [1.21.0] - 2026-02-25

** Added
- YAML frontmatter sync to STATE.md for machine-readable status tracking
- ~/gmsd:add-tests~ command for post-phase test generation
- Codex runtime support with skills-first installation
- Standard ~project_context~ block in gmsd-verifier output
- Codex changelog and usage documentation

** Changed
- Improved onboarding UX: installer now suggests ~/gmsd:new-project~ instead of ~/gmsd:help~
- Updated Discord invite to vanity URL (discord.gg/gsd)
- Compressed Nyquist validation layer to align with GMSD meta-prompt conventions
...
- ~/gmsd:update~ always installs latest package version
...
** Fixed
- ~/gmsd:health --repair~ now creates timestamped backup before regenerating STATE.md (#657)
...
** Added
- ~/gmsd:health~ command — validates ~.planning/~ directory integrity with ~--repair~ flag for auto-fixing config.json and STATE.md
- ~--full~ flag for ~/gmsd:quick~ — enables plan-checking (max 2 iterations) and post-execution verification on quick tasks
- ~--auto~ flag wired from ~/gmsd:new-project~ through the full phase chain (discuss → plan → execute)
- Auto-advance chains phase execution across full milestones when ~workflow.auto_advance~ is enabled

** Fixed
- Plans created without user context — ~/gmsd:plan-phase~ warns when no CONTEXT.md exists, ~/gmsd:discuss-phase~ warns when plans already exist (#253)
- OpenCode installer converts ~general-purpose~ subagent type to OpenCode's ~general~
- ~/gmsd:complete-milestone~ respects ~commit_docs~ setting when merging branches
- Phase directories tracked in git via ~.gitkeep~ files

* [1.19.2] - 2026-02-15

** Added
- User-level default settings via ~~/.gmsd/defaults.json~ — set GMSD defaults across all projects
...
- ESM conflicts prevented by renaming gmsd-tools.js to .cjs
...
** Added
- ~--auto~ flag for ~/gmsd:new-project~ — runs research → requirements → roadmap automatically after config questions. Expects idea document via @ reference (e.g., ~/gmsd:new-project --auto @prd.md~)
...
- Research decision from ~/gmsd:new-milestone~ now persists to config.json

* [1.17.0] - 2026-02-08

** Added
- *gmsd-tools verification suite*: ~verify plan-structure~, ~verify phase-completeness~, ~verify references~, ~verify commits~, ~verify artifacts~, ~verify key-links~ — deterministic structural checks
- *gmsd-tools frontmatter CRUD*: ~frontmatter get/set/merge/validate~ — safe YAML frontmatter operations with schema validation
- *gmsd-tools template fill*: ~template fill summary/plan/verification~ — pre-filled document skeletons
- *gmsd-tools state progression*: ~state advance-plan~, ~state update-progress~, ~state record-metric~, ~state add-decision~, ~state add-blocker~, ~state resolve-blocker~, ~state record-session~ — automates STATE.md updates
- *Local patch preservation*: Installer now detects locally modified GMSD files, backs them up to ~gmsd-local-patches/~, and creates a manifest for restoration
- ~/gmsd:reapply-patches~ command to merge local modifications back after GMSD updates

** Changed
- Agents (executor, planner, plan-checker, verifier) now use gmsd-tools for state updates and verification instead of manual markdown parsing
- ~/gmsd:update~ workflow now notifies about backed-up local patches and suggests ~/gmsd:reapply-patches~
...
** Added
- 10 new gmsd-tools CLI commands that replace manual AI orchestration of mechanical operations:
...
** Changed
- Workflows now delegate deterministic operations to gmsd-tools CLI, reducing token usage and errors:
...
** Fixed
- Execute-phase now correctly spawns ~gmsd-executor~ subagents instead of generic task agents
- ~commit_docs=false~ setting now respected in all ~.planning/~ commit paths (execute-plan, debugger, reference docs all route through gmsd-tools CLI)
...
- Windows: Normalized backslash paths in gmsd-tools invocations (contributed by @rmindel)
...
** Added
- Context-optimizing parsing commands in gmsd-tools (~phase-plan-index~, ~state-snapshot~, ~summary-extract~) — reduces agent context usage by returning structured JSON instead of raw file content
...
** Added
- ~gmsd-tools history-digest~ — Compiles phase summaries into structured JSON for faster context loading
- ~gmsd-tools phases list~ — Lists phase directories with filtering (replaces fragile ~ls | sort -V~ patterns)
- ~gmsd-tools roadmap get-phase~ — Extracts phase sections from ROADMAP.md
- ~gmsd-tools phase next-decimal~ — Calculates next decimal phase number for insert operations
- ~gmsd-tools state get/patch~ — Atomic STATE.md field operations
- ~gmsd-tools template select~ — Chooses summary template based on plan complexity
...
- Agents migrated from bash patterns to structured gmsd-tools commands
...
- *Centralized utilities* — New ~gmsd-tools.cjs~ (11 functions) replaces repetitive bash patterns across 50+ files
...
** Added
- ~gmsd-tools.cjs~ CLI utility with functions: state load/update, resolve-model, find-phase, commit, verify-summary, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section
...
** Fixed
- *CRITICAL:* Prevent API keys from being committed via ~/gmsd:map-codebase~ (#429)
...
- Auto-create ~config.json~ when missing during ~/gmsd:settings~ (#264)
- ~/gmsd:update~ respects local vs global install location
...
- Removed broken gmsd-gemini link (404)
...
- Git branching strategy configuration with three options:
  ...
  - ~phase~: create branch per phase (~gmsd/phase-{N}-{slug}~)
...
** Fixed
- CONTEXT.md from ~/gmsd:discuss-phase~ now properly flows to all downstream agents (researcher, planner, checker, revision loop)
...
** Removed
- ~/gmsd:whats-new~ command — use ~/gmsd:update~ instead (shows changelog with cancel option)
...
** Added
- ~/gmsd:join-discord~ command to quickly access the GMSD Discord community invite link

* [1.9.8] - 2025-01-22

** Added
- Uninstall flag (~--uninstall~) to cleanly remove GMSD from global or local installations
...
- OpenCode commands use flat structure (~command/gmsd-help.md~) matching OpenCode's expected format
...
- Native OpenCode support: ~--opencode~ flag converts GMSD to OpenCode format automatically
- ~--both~ flag to install for both Claude Code and OpenCode in one command
- Auto-configures ~~/.opencode.json~ permissions for seamless GMSD doc access
...
- Removed stray backticks in ~/gmsd:new-project~ output
...
- *Codebase Intelligence System* — Removed due to overengineering concerns
  - Deleted ~/gmsd:analyze-codebase~ command
  - Deleted ~/gmsd:query-intel~ command
  - Removed SQLite graph database and sql.js dependency (21MB)
  - Removed intel hooks (gmsd-intel-index.js, gmsd-intel-session.js, gmsd-intel-prune.js)
...
** Added
- *Model Profiles* — ~/gmsd:set-profile~ for quality/balanced/budget agent configurations
- *Workflow Settings* — ~/gmsd:settings~ command for toggling workflow behaviors interactively
...
- All hooks now use ~gmsd-~ prefix for consistency (statusline.js → gmsd-statusline.js)
...
- ~/gmsd:new-project~ now asks about git tracking during initial setup, letting you opt out of committing planning docs from the start
...
** Added
- *Quick Mode* (~/gmsd:quick~) — Execute small, ad-hoc tasks with GMSD guarantees but skip optional agents (researcher, checker, verifier). Quick tasks live in ~.planning/quick/~ with their own tracking in STATE.md.

** Changed
- Improved progress bar calculation to clamp values within 0-100 range
- Updated documentation with comprehensive Quick Mode sections in help.md, README.md, and GMSD-STYLE.md
...
- Removed dead gmsd-researcher agent references
...
- Orphaned ~gmsd-notify.sh~ hook from previous versions is now automatically removed during install (both file and settings.json registration)

* [1.6.3] - 2025-01-17

** Added
- ~--gaps-only~ flag for ~/gmsd:execute-phase~ — executes only gap closure plans after verify-work finds issues, eliminating redundant state discovery
...
** Changed
- Installer performs clean install of GMSD folders, removing orphaned files from previous versions
- ~/gmsd:update~ shows changelog and asks for confirmation before updating, with clear warning about what gets replaced

* [1.6.0] - 2026-01-17

** Changed
- *BREAKING:* Unified ~/gmsd:new-milestone~ flow — now mirrors ~/gmsd:new-project~ with questioning → research → requirements → roadmap in a single command
- Roadmapper agent now references templates instead of inline structures for easier maintenance

** Removed
- *BREAKING:* ~/gmsd:discuss-milestone~ — consolidated into ~/gmsd:new-milestone~
- *BREAKING:* ~/gmsd:create-roadmap~ — integrated into project/milestone flows
- *BREAKING:* ~/gmsd:define-requirements~ — integrated into project/milestone flows
- *BREAKING:* ~/gmsd:research-project~ — integrated into project/milestone flows

** Added
- ~/gmsd:verify-work~ now includes next-step routing after verification completes
...
- Next-step suggestions now consistently recommend ~/gmsd:discuss-phase~ before ~/gmsd:plan-phase~ across all routing paths
...
- Removed blocking notification popups (gmsd-notify) on all platforms
...
- *BREAKING:* Removed ~/gmsd:execute-plan~ command (use ~/gmsd:execute-phase~ instead)
...
- Removed ~/gmsd:research-phase~ from next-step suggestions (use ~/gmsd:plan-phase~ instead)

* [1.5.22] - 2025-01-16

** Added
- Statusline update indicator — shows ~⬆ /gmsd:update~ when a new version is available
...
** Added
- GMSD brand system for consistent UI (checkpoint boxes, stage banners, status symbols)
- Research synthesizer agent that consolidates parallel research into SUMMARY.md

** Changed
- *Unified ~/gmsd:new-project~ flow* — Single command now handles questions → research → requirements → roadmap (~10 min)
- Simplified README to reflect streamlined workflow: new-project → plan-phase → execute-phase
- Added optional ~/gmsd:discuss-phase~ documentation for UI/UX/behavior decisions before planning
...
** Changed
- ~/gmsd:discuss-phase~ redesigned with intelligent gray area analysis — analyzes phase to identify discussable areas (UI, UX, Behavior, etc.), presents multi-select for user control, deep-dives each area with focused questioning
...
- ~/gmsd:plan-phase~ now integrates research — spawns ~gmsd-phase-researcher~ before planning unless research exists or ~--skip-research~ flag used

* [1.5.18] - 2026-01-16

** Added
- *Plan verification loop* — Plans are now verified before execution with a planner → checker → revise cycle
  - New ~gmsd-plan-checker~ agent (744 lines) validates plans will achieve phase goals
...
- *Dedicated planner agent* — ~gmsd-planner~ (1,319 lines) consolidates all planning expertise
...
** Changed
- ~/gmsd:plan-phase~ refactored to thin orchestrator pattern (310 lines)
  - Spawns ~gmsd-planner~ for planning, ~gmsd-plan-checker~ for verification
  - User sees status between agent spawns (not a black box)
- Planning references deprecated with redirects to ~gmsd-planner~ agent sections
  - ~plan-format.md~, ~scope-estimation.md~, ~goal-backward.md~, ~principles.md~
  - ~workflows/plan-phase.md~

** Fixed
- Removed zombie ~gmsd-milestone-auditor~ agent (was accidentally re-added after correct deletion)
...
** Added
- New ~/gmsd:update~ command — check for updates, install, and display changelog of what changed (better UX than raw ~npx get-my-shit-done-cc~)

* [1.5.16] - 2026-01-15

** Added
- New ~gmsd-researcher~ agent (915 lines) with comprehensive research methodology, 4 research modes (ecosystem, feasibility, implementation, comparison), source hierarchy, and verification protocols
- New ~gmsd-debugger~ agent (990 lines) with scientific debugging methodology, hypothesis testing, and 7+ investigation techniques
- New ~gmsd-codebase-mapper~ agent for brownfield codebase analysis
- Research subagent prompt template for context-only spawning

** Changed
- ~/gmsd:research-phase~ refactored to thin orchestrator — now injects rich context (key insight framing, downstream consumer info, quality gates) to gmsd-researcher agent
- ~/gmsd:research-project~ refactored to spawn 4 parallel gmsd-researcher agents with milestone-aware context (greenfield vs v1.1+) and roadmap implications guidance
- ~/gmsd:debug~ refactored to thin orchestrator (149 lines) — spawns gmsd-debugger agent with full debugging expertise
- ~/gmsd:new-milestone~ now explicitly references MILESTONE-CONTEXT.md

** Deprecated
- ~workflows/research-phase.md~ — consolidated into gmsd-researcher agent
- ~workflows/research-project.md~ — consolidated into gmsd-researcher agent
- ~workflows/debug.md~ — consolidated into gmsd-debugger agent
- ~references/research-pitfalls.md~ — consolidated into gmsd-researcher agent
- ~references/debugging.md~ — consolidated into gmsd-debugger agent
- ~references/debug-investigation.md~ — consolidated into gmsd-debugger agent

* [1.5.15] - 2025-01-15

** Fixed
- *Agents now install correctly* — The ~agents/~ folder (gmsd-executor, gmsd-verifier, gmsd-integration-checker, gmsd-milestone-auditor) was missing from npm package, now included

** Changed
- Consolidated ~/gmsd:plan-fix~ into ~/gmsd:plan-phase --gaps~ for simpler workflow
- UAT file writes now batched instead of per-response for better performance

* [1.5.14] - 2025-01-15

** Fixed
- Plan-phase now always routes to ~/gmsd:execute-phase~ after planning, even for single-plan phases

* [1.5.13] - 2026-01-15

** Fixed
- ~/gmsd:new-milestone~ now presents research and requirements paths as equal options, matching ~/gmsd:new-project~ format
...
- ~progress~ now correctly routes to ~/gmsd:discuss-milestone~ when between milestones (Route F)
...
** Added
- Milestone audit system (~/gmsd:audit-milestone~) for verifying milestone completion with parallel verification agents
...
- Execute-phase now recommends ~/gmsd:audit-milestone~ when milestone completes
...
** Changed
- ~gmsd-executor~ subagent color changed from red to blue

* [1.5.7] - 2025-01-15

** Added
- ~gmsd-executor~ subagent: Dedicated agent for plan execution with full workflow logic built-in
- ~gmsd-verifier~ subagent: Goal-backward verification that checks if phase goals are actually achieved (not just tasks completed)
...
** Changed
- execute-plan and execute-phase now spawn ~gmsd-executor~ subagent instead of using inline workflow
...
** Fixed
- Updated remaining ~general-purpose~ subagent references to use ~gmsd-executor~
...
** Removed
- Dead ~/gmsd:status~ command (referenced abandoned background agent model)
...
** Added
- New ~/gmsd:research-project~ command for pre-roadmap ecosystem research — spawns parallel agents to investigate stack, features, architecture, and pitfalls before you commit to a roadmap
- New ~/gmsd:define-requirements~ command for scoping v1 requirements from research findings — transforms "what exists in this domain" into "what we're building"
...
** Added
- New ~/gmsd:whats-new~ command shows changes since your installed version
...
** Added
- ~/gmsd:debug~ command for systematic debugging with persistent state
...
** Added
- Parallel phase execution via ~/gmsd:execute-phase~
- Parallel-aware planning in ~/gmsd:plan-phase~
- ~/gmsd:status~ command for parallel agent monitoring
...
** Added
- ~/gmsd:add-todo~ and ~/gmsd:check-todos~ for mid-session idea capture
...
** Added
- ~/gmsd:resume-task~ for resuming interrupted subagent executions
...
** Added
- ~/gmsd:verify-work~ for conversational UAT validation
- ~/gmsd:plan-fix~ for fixing UAT issues
- UAT issues template
...
** Added
- ~/gmsd:map-codebase~ documentation in help and README

* [1.3.0] - 2025-12-17

** Added
- ~/gmsd:map-codebase~ command for brownfield project analysis
...
- Brownfield integration into GMSD workflows
...
- ~/gmsd:research-phase~ for niche domain ecosystem discovery
- ~/gmsd:research-project~ command with workflow and templates
- ~/gmsd:create-roadmap~ command with research-aware workflow
...
** Added
- Initial release of GMSD (Get Shit Done) meta-prompting system
- Core slash commands: ~/gmsd:new-project~, ~/gmsd:discuss-phase~, ~/gmsd:plan-phase~, ~/gmsd:execute-phase~
