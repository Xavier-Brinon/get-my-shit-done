...
```bash
PHASE_INFO=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs find-phase "${PHASE}" --raw)
```
