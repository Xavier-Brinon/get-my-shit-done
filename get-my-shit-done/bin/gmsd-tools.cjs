...
/**
 * GMSD Tools — CLI utility for GMSD workflow operations
 *
 * Replaces repetitive inline bash patterns across ~50 GMSD command/workflow/agent files.
 * Centralizes: config parsing, model resolution, phase lookup, git commits, summary verification.
 *
 * Usage: node gmsd-tools.cjs <command> [args] [--raw]
 *
 * Atomic Commands:
...
async function main() {
  ...
  if (!command) {
    error('Usage: gmsd-tools <command> [args] [--raw] [--cwd <path>]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
