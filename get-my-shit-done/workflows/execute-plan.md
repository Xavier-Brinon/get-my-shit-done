...
# config settings can be fetched via gmsd-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gmsd-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phases list --type summaries --raw
...
See `~/.claude/get-my-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-my-shit-done/references/checkpoints.md for details.
</step>
...
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gmsd-tools:
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gmsd:execute-phase {phase}` + `/gmsd:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gmsd:plan-phase {Z+1}` + `/gmsd:verify-work {Z}` + `/gmsd:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gmsd:complete-milestone` + `/gmsd:verify-work` + `/gmsd:add-phase` |
