...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init plan-phase "$PHASE")
```
...
**If `planning_exists` is false:** Error — run `/gmsd:new-project` first.
...
```bash
PHASE_INFO=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
If "Run discuss-phase first": Display `/gmsd:discuss-phase {X}` and exit workflow.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GMSD ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn gmsd-phase-researcher

```bash
PHASE_DESC=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```
Task(
  prompt="First, read ~/.claude/agents/gmsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. Read validation template from `~/.claude/get-my-shit-done/templates/VALIDATION.org`
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
## 8. Spawn gmsd-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GMSD ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /gmsd:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ~/.claude/agents/gmsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn gmsd-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GMSD ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  ...
  subagent_type="gmsd-plan-checker",
  model="{checker_model}",
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GMSD ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  prompt="
    ...
    <execution_context>
    @~/.claude/get-my-shit-done/workflows/execute-phase.md
    @~/.claude/get-my-shit-done/references/checkpoints.md
    @~/.claude/get-my-shit-done/references/tdd.md
    @~/.claude/get-my-shit-done/references/model-profile-resolution.md
    </execution_context>
    ...
    4. When spawning executor agents, use subagent_type='gmsd-executor' with the existing @file pattern from the workflow
    5. When spawning verifier agents, use subagent_type='gmsd-verifier'
    6. Preserve the classifyHandoffIfNeeded workaround (spot-check on that specific error)
    7. Do NOT use the Skill tool or /gmsd: commands
    </instructions>
  ",
  subagent_type="general-purpose",
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   GMSD ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /gmsd:discuss-phase ${NEXT_PHASE} --auto
  ```
