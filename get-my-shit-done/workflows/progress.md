...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init progress)
```
...
Run /gmsd:new-project to start a new project.
```
...
<step name="load">
**Use structured extraction from gmsd-tools:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs roadmap analyze)`
- `STATE=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs state-snapshot)`
...
<step name="report">
**Generate progress bar from gmsd-tools, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs progress bar --raw)
```
...
## Pending Todos
- [count] pending — /gmsd:check-todos to review

## Active Debug Sessions
- [count] active — /gmsd:debug to continue
...
**Also available:**
- `/gmsd:plan-phase {phase}` — skip discussion, plan directly
- `/gmsd:list-phase-assumptions {phase}` — see Claude's assumptions
...
**Also available:**
- `/gmsd:execute-phase {phase}` — execute phase plans
- `/gmsd:verify-work {phase}` — run more UAT testing
...
**Also available:**
- `/gmsd:plan-phase {Z+1}` — skip discussion, plan directly
- `/gmsd:verify-work {Z}` — user acceptance test before continuing
...
**Also available:**
- `/gmsd:verify-work` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/gmsd:plan-phase [next]`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/gmsd:resume-work`
  </step>
...
- [ ] Smart routing: /gmsd:execute-phase if plans exist, /gmsd:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate gmsd command
      </success_criteria>
