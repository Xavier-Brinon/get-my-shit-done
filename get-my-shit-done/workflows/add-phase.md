...
- Example: `/gmsd:add-phase Add authentication` → description = "Add authentication"
- Example: `/gmsd:add-phase Fix critical performance issues` → description = "Fix critical performance issues"

If no arguments provided:

```
ERROR: Phase description required
Usage: /gmsd:add-phase <description>
Example: /gmsd:add-phase Add authentication system
```
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init phase-op "0")
```
...
```bash
RESULT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phase add "${description}")
```
...
**Also available:**
- `/gmsd:add-phase <description>` — add another phase
- Review roadmap
