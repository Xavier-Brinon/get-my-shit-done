...
**Use the transition result from `gmsd-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/gmsd:discuss-phase [X+1] --auto")
...
**Also available:**
- `/gmsd:plan-phase [X+1]` — skip discussion, plan directly
- `/gmsd:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/gmsd:discuss-phase [X+1]` — revisit context
- `/gmsd:research-phase [X+1]` — investigate unknowns
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs config-set workflow.auto_advance false
```
...
Exit skill and invoke SlashCommand("/gmsd:complete-milestone {version}")
