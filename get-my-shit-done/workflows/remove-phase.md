...
- Example: `/gmsd:remove-phase 17` → phase = 17
- Example: `/gmsd:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /gmsd:remove-phase <phase-number>
Example: /gmsd:remove-phase 17
```
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init phase-op "${target}")
```
...
To abandon current work, use /gmsd:pause-work instead.
```
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/gmsd:progress` — see updated roadmap status
...
- [ ] `gmsd-tools phase remove` executed successfully
