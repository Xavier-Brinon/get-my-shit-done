<purpose>
Check for GMSD updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether GMSD is installed locally or globally by checking both locations and validating install integrity:

```bash
# Check local first (takes priority only if valid)
# Paths templated at install time for runtime compatibility
LOCAL_VERSION_FILE="./.claude/get-my-shit-done/VERSION"
LOCAL_MARKER_FILE="./.claude/get-my-shit-done/workflows/update.md"
GLOBAL_VERSION_FILE="$HOME/.claude/get-my-shit-done/VERSION"
GLOBAL_MARKER_FILE="$HOME/.claude/get-my-shit-done/workflows/update.md"
...
```bash
npm view get-my-shit-done-cc version 2>/dev/null
```
...
To update manually: `npx get-my-shit-done-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of GMSD folders:
- `commands/gmsd/` will be wiped and replaced
- `get-my-shit-done/` will be wiped and replaced
...
If you've modified any GMSD files directly, they'll be automatically backed up to `gmsd-local-patches/` and can be reapplied with `/gmsd:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y get-my-shit-done-cc@latest --local
```

**If GLOBAL install (or unknown):**
```bash
npx -y get-my-shit-done-cc@latest --global
```
...
**If LOCAL install:**
```bash
rm -f ./.claude/cache/gmsd-update-check.json
```
