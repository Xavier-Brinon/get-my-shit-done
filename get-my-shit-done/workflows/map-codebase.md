...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init map-codebase)
```
...
Spawn 4 parallel gmsd-codebase-mapper agents.

Use Task tool with `subagent_type="gmsd-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `gmsd-codebase-mapper` agent, NOT `Explore`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="gmsd-codebase-mapper",
  model="{mapper_model}",
...
- 4 parallel gmsd-codebase-mapper agents spawned with run_in_background=true
- Agents write documents directly (orchestrator doesn't receive document contents)
- Read agent output files to collect confirmations
- All 7 codebase documents exist
- Clear completion summary with line counts
- User offered clear next steps in GMSD style
</success_criteria>
