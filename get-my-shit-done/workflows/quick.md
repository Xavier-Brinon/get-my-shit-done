...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init quick "$DESCRIPTION")
```
...
Spawn gmsd-executor with plan reference:
...
GMSD > QUICK TASK COMPLETE (FULL MODE)
...
Ready for next task: /gmsd:quick
```
