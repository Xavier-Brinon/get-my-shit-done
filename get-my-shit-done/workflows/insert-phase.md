...
Example: `/gmsd:insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /gmsd:insert-phase <after> <description>
Example: /gmsd:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phase insert "${after_phase}" "${description}")
```
...
- Don't use this for planned work at end of milestone (use /gmsd:add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /gmsd:plan-phase)
