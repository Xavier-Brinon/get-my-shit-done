...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init execute-phase "${PHASE_ARG}")
```
...
```bash
PLAN_INDEX=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       <execution_context>
       @~/.claude/get-my-shit-done/workflows/execute-plan.md
       @~/.claude/get-my-shit-done/templates/summary.org
...
```bash
PARENT_INFO=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs find-phase "${PARENT_PHASE}" --raw)
...
| `gaps_found` | Present gap summary, offer `/gmsd:plan-phase {phase} --gaps` |
...
```bash
COMPLETION=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phase complete "${PHASE_NUMBER}")
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/gmsd:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/get-my-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_CFG` is true:**

The workflow ends. The user runs `/gmsd:progress` or invokes the transition workflow manually.
</step>
