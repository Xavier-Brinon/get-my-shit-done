...
```
No audit gaps found. Run `/gmsd:audit-milestone` first.
```
...
PHASES=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs phases list)
...
`/gmsd:audit-milestone` — re-audit to verify gaps closed
`/gmsd:complete-milestone {version}` — archive when audit passes
```
