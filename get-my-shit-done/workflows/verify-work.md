...
<template>
@~/.claude/get-my-shit-done/templates/UAT.org
</template>
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init verify-work "${PHASE_ARG}")
```
...
Spawn gmsd-planner in --gaps mode:
...
Spawn gmsd-planner with revision context:
...
- [ ] If issues: gmsd-planner creates fix plans (gap_closure mode)
- [ ] If issues: gmsd-plan-checker verifies fix plans
