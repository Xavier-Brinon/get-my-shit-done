...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/gmsd:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init phase-op "${PHASE}")
```
...
Use /gmsd:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /gmsd:plan-phase {X} to replan
...
Task(
  prompt="
    ...
    3. When spawning agents (gmsd-phase-researcher, gmsd-planner, gmsd-plan-checker), use Task with specified subagent_type and model
    ...
    5. Do NOT use the Skill tool or /gmsd: commands. Read workflow .md files directly.
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /gmsd:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /gmsd:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /gmsd:plan-phase ${PHASE} --gaps
  ```
