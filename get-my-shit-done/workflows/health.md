...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs validate health $REPAIR_FLAG
```
...
```
---
N issues can be auto-repaired. Run: /gmsd:health --repair
```
</step>
...
```
Would you like to run /gmsd:health --repair to fix N issues automatically?
```
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs validate health
```
