<purpose>
Interactive configuration of GMSD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.gmsd/defaults.json) for future projects.
</purpose>
...
```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs config-ensure-section
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.gmsd/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.gmsd/defaults.json`:

```bash
mkdir -p ~/.gmsd
```

Write `~/.gmsd/defaults.json` with:
```json
{
  "mode": <current>,
  "depth": <current>,
...
Quick commands:
- /gmsd:set-profile <profile> — switch model profile
- /gmsd:plan-phase --research — force research
- /gmsd:plan-phase --skip-research — skip research
- /gmsd:plan-phase --skip-verify — skip plan check
```
</step>
