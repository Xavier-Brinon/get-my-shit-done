...
- File reference: `/gmsd:new-project --auto @prd.md`
...
Usage:
  /gmsd:new-project --auto @your-idea.md
  /gmsd:new-project --auto [paste or write your idea here]
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init new-project)
```

Parse JSON for: `researcher_model`, `synthesizer_model`, `roadmapper_model`, `commit_docs`, `project_exists`, `has_codebase_map`, `planning_exists`, `has_existing_code`, `has_package_file`, `is_brownfield`, `needs_codebase_map`, `has_git`, `project_path`.

**If `project_exists` is true:** Error — project already initialized. Use `/gmsd:progress`.
...
- options:
  - "Map codebase first" — Run /gmsd:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/gmsd:map-codebase` first, then return to `/gmsd:new-project`
```
Exit command.
...
```bash
mkdir -p .planning
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs commit "chore: add project config" --files .planning/config.json
```

**Persist auto-advance to config (survives context compaction):**

```bash
node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs config-set workflow.auto_advance true
```
...
**Check for global defaults** at `~/.gmsd/defaults.json`. If the file exists, offer to use saved defaults:

```
AskUserQuestion([
  {
    question: "Use your saved default settings? (from ~/.gmsd/defaults.json)",
    header: "Defaults",
    multiSelect: false,
    options: [
...
If "Yes": read `~/.gmsd/defaults.json`, use those values for config.json, and skip directly to **Commit config.json** below.

If "No" or `~/.gmsd/defaults.json` doesn't exist: proceed with the questions below.
...
**Note:** Run `/gmsd:settings` anytime to update these preferences.
...
Spawn 4 parallel gmsd-project-researcher agents with path references:

```
Task(prompt="First, read ~/.claude/agents/gmsd-project-researcher.md for your role and instructions.
...
Spawn gmsd-roadmapper agent with path references:
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="gmsd-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
Exit skill and invoke SlashCommand("/gmsd:discuss-phase 1 --auto")
...
**Also available:**
- /gmsd:plan-phase 1 — skip discussion, plan directly
