...
:diataxis_rationale: Reader (planner agent) consults this template to look up the required PLAN.org output format, frontmatter fields, and task structure to produce correct executable plans.
:diataxis_meta: true
:END:
...
> *Note:* Planning methodology is in ~agents/gmsd-planner.md~.
> This template defines the PLAN.org output format that the agent produces.

Template for ~.planning/phases/XX-name/{phase}-{plan}-PLAN.org~ - executable phase plans optimized for parallel execution.

*Naming:* Use ~{phase}-{plan}-PLAN.org~ format (e.g., ~01-02-PLAN.org~ for Phase 1, Plan 2)
...
<!-- For checkpoint task examples and patterns, see @~/.claude/get-my-shit-done/references/checkpoints.md -->
...
*Wave is pre-computed:* Wave numbers are assigned during ~/gmsd:plan-phase~. Execute-phase reads ~wave~ directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
#+begin_src org
<context>
@.planning/phases/03-features/03-01-SUMMARY.org  # Just because it's earlier
@.planning/phases/03-features/03-02-SUMMARY.org  # Reflexive chaining
</context>
#+end_src
...
See ~~/.claude/get-my-shit-done/references/tdd.md~ for TDD plan structure.
...
<output>
After completion, create =.planning/phases/03-features/03-01-SUMMARY.org=
</output>
#+end_src
...
@.planning/PROJECT.org
@.planning/ROADMAP.org
@.planning/phases/03-features/03-01-SUMMARY.org
@.planning/phases/03-features/03-02-SUMMARY.org
</context>
...
<output>
After completion, create =.planning/phases/03-features/03-03-SUMMARY.org=
</output>
#+end_src
...
*Result:* Execute-plan generates ~{phase}-USER-SETUP.org~ with checklist for the user.

See ~~/.claude/get-my-shit-done/templates/user-setup.org~ for full schema and examples
...
2. Must_haves written to PLAN.org frontmatter
3. Execute-phase runs all plans
4. Verification subagent checks must_haves against codebase
5. Gaps found → fix plans created → execute → re-verify
6. All must_haves pass → phase complete

See ~~/.claude/get-my-shit-done/workflows/verify-phase.md~ for verification logic.
