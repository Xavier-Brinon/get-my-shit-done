...
:diataxis_rationale: Reader (debugger agent) consults the generated debug file to look up current hypothesis, eliminated theories, and evidence accumulated so far — internal active session state.
:diataxis_meta: true
:END:
...
Template for ~.planning/debug/[slug].org~ — active debug session tracking.
...
*Creation:* Immediately when /gmsd:debug is called
