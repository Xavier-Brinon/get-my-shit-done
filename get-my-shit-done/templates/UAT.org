...
:diataxis_rationale: Reader (UAT agent) consults the generated UAT.org to look up current test status, which test is active, and which gaps have been found — internal active testing session state.
:diataxis_meta: true
:END:
...
Template for ~.planning/phases/XX-name/{phase_num}-UAT.org~ — persistent UAT session tracking.
...
:phase: XX-name
:source: [list of SUMMARY.org files tested]
...
- This section feeds directly into /gmsd:plan-phase --gaps
...
4. UAT.org Gaps section updated with diagnosis:
   - Each gap gets ~root_cause~, ~artifacts~, ~missing~, ~debug_session~ filled
5. status → "diagnosed"
6. Ready for /gmsd:plan-phase --gaps with root causes
...
*Creation:* When /gmsd:verify-work starts new session
- Extract tests from SUMMARY.org files
...
:status: diagnosed
:phase: 04-comments
:source: 04-01-SUMMARY.org, 04-02-SUMMARY.org
