...
:diataxis_rationale: Reader consults the generated STACK.org to look up which languages, runtimes, frameworks, and key dependencies the project uses.
:END:
...
Template for ~.planning/codebase/STACK.org~ - captures the technology foundation.
...
- File structure (that's STRUCTURE.org)
- Architectural patterns (that's ARCHITECTURE.org)
