...
:diataxis_rationale: Reader consults the generated TESTING.org to look up the test framework, file conventions, mocking patterns, and commands needed to add matching tests.
:END:
...
Template for ~.planning/codebase/TESTING.org~ - captures test framework and patterns.
