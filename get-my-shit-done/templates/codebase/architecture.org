...
:diataxis_rationale: Reader consults the generated ARCHITECTURE.org to look up how the codebase is structured at a conceptual level — layers, data flow, and abstractions.
:END:
...
Template for ~.planning/codebase/ARCHITECTURE.org~ - captures conceptual code organization.

*Purpose:* Document how the code is organized at a conceptual level. Complements STRUCTURE.org (which shows physical file locations).
...
- Examples: PROJECT.org, PLAN.org templates
...
- Triggers: User runs =gmsd <command>=
...
#+CAPTION: Architecture Layers (MermaidJS ASCII)
#+BEGIN_EXAMPLE
┌─────────────────────────────────┐
│          Command Layer          │
│                                 │
...
flowchart TD
    subgraph Presentation["Command Layer"]
        ...
        CMD["Commands: src/commands/*.ts"]
    end
    subgraph Business["Service Layer"]
        SVC["Services: src/services/*.ts"]
    end
    subgraph Infrastructure["Utility Layer"]
        UTL["Utilities: src/utils/*.ts"]
        FS["File System"]
    end
    CLI --> CMD
    CMD --> SVC
...
#+CAPTION: Architecture Data Flow (MermaidJS ASCII)
#+BEGIN_EXAMPLE
┌──────┐            ┌───────────┐      ┌─────────────────┐   ┌─────────┐           ┌─────────────┐
│ User │            │ CLI Entry │      │ Command Handler │   │ Service │           │ File System │
└───┬──┘            └─────┬─────┘      └────────┬────────┘   └────┬────┘           └──────┬──────┘
    │                     │                     │                 │                       │
    │  gmsd new-project   │                     │                 │                       │
    │─────────────────────▶                     │                 │                       │
...
sequenceDiagram
    ...
    U->>C: gmsd new-project
    C->>CMD: Route to handler
    CMD->>SVC: create()
    SVC->>FS: Read templates
    FS-->>SVC: Template content
    SVC->>FS: Write output files
    SVC-->>CMD: Result
    CMD-->>U: Console output
#+end_src
...
<guidelines>
*What belongs in ARCHITECTURE.org:*
...
- Exhaustive file listings (that's STRUCTURE.org)
- Technology choices (that's STACK.org)
