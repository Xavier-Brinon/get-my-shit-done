...
:diataxis_rationale: Reader consults the generated INTEGRATIONS.org to look up which external services are used, where credentials live, and how connections are configured.
:END:
...
Template for ~.planning/codebase/INTEGRATIONS.org~ - captures external service dependencies.
...
<guidelines>
*What belongs in INTEGRATIONS.org:*
...
- Internal architecture (that's ARCHITECTURE.org)
- Code patterns (that's PATTERNS.org)
- Technology choices (that's STACK.org)
- Performance issues (that's CONCERNS.org)
