...
:diataxis_rationale: Reader consults the generated CONVENTIONS.org to look up naming patterns, formatting rules, and code style to match when writing new code.
:END:
...
Template for ~.planning/codebase/CONVENTIONS.org~ - captures coding style and patterns.
...
<guidelines>
*What belongs in CONVENTIONS.org:*
...
- Architecture decisions (that's ARCHITECTURE.org)
- Technology choices (that's STACK.org)
- Test patterns (that's TESTING.org)
- File organization (that's STRUCTURE.org)
