...
:diataxis_rationale: Reader consults the generated VERIFICATION.org to look up which must-haves were verified, which artifacts exist, and what gaps were found after phase execution.
:END:
...
Template for ~.planning/phases/XX-name/{phase_num}-VERIFICATION.org~ — phase goal verification results.
...
*** {phase}-{next+1}-PLAN.org: {Fix Name}
...
*Must-haves source:* {PLAN.org frontmatter | derived from ROADMAP.org goal}
...
#+title: Phase 3: Chat Interface Verification Report
#+phase_goal: Working chat interface where users can send and receive messages
#+verified: [2025-01-15 Wed 14:30]
#+status: gaps_found
#+startup: indent
#+options: toc:2 num:nil ^:{}
...
*** 03-04-PLAN.org: Implement Chat API
...
*** 03-05-PLAN.org: Implement Chat UI
...
*Verification approach:* Goal-backward (derived from phase goal)
*Must-haves source:* 03-01-PLAN.org frontmatter
