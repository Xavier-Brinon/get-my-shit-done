...
:diataxis_rationale: Reader consults the generated milestone archive to look up what was delivered, which phases were completed, and key decisions made during a shipped milestone.
:END:
...
#+title: Milestone v{{VERSION}}: {{MILESTONE_NAME}}
#+status: SHIPPED {{DATE}}
...
- Decision: Use ROADMAP.org split (Rationale: Constant context cost)
...
- PROJECT-STATE.org tiering (deferred until decisions > 300)
...
_For current project status, see .planning/ROADMAP.org_
...
- Extract phase details from ROADMAP.org
- Document decimal phases with (INSERTED) marker
- Include key decisions from PROJECT-STATE.org or SUMMARY files
- List issues resolved vs deferred
- Capture technical debt for future reference

*Archive location:*

- Save to ~.planning/milestones/v{VERSION}-{NAME}.org~
- Example: ~.planning/milestones/v1.0-mvp.org~

*After archiving:*

- Update ROADMAP.org to collapse completed milestone in ~<details>~ tag
- Update PROJECT.org to brownfield format with Current State section
