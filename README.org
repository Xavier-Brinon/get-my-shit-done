...
*A customised fork of [[https://github.com/glittercowboy/get-shit-done][Get Shit Done (GSD)]] — the meta-prompting, context engineering, and spec-driven development system for Claude Code.*
...
Everything below is adapted from the upstream GSD project. For the original documentation, community, and support, visit [[https://github.com/glittercowboy/get-shit-done][github.com/glittercowboy/get-shit-done]].
...
*NOTE:* Codex installation uses skills (~skills/gmsd-*/SKILL.md~) rather than custom prompts.
...
*TIP:* This is how GMSD is intended to be used — stopping to approve ~date~ and ~git commit~ 50 times defeats the purpose.
...
GMSD is designed to work with Claude Code's ~--worktree~ (~-w~) flag. Each phase runs in an isolated worktree with its own branch — no cross-contamination between phases, clean PRs, easy rollback.
...
When ~/gmsd:execute-phase 2~ runs inside that worktree, it detects it's already on a non-main branch and skips its own branch creation. All commits go to the worktree branch.

When you're ready to complete the milestone, ~/gmsd:complete-milestone~ discovers both ~gmsd/phase-*~ and ~worktree-phase-*~ branches and offers merge options (squash merge recommended).
...
*Creates:* ~PROJECT.org~, ~REQUIREMENTS.org~, ~ROADMAP.org~, ~STATE.org~, ~.planning/research/~
...
For each area you select, it asks until you're satisfied. The output — ~CONTEXT.org~ — feeds directly into the next two steps:
...
1. *Researches* — Investigates how to implement this phase, guided by your ~CONTEXT.org~ decisions
...
GMSD stores project settings in ~.planning/config.json~. Configure during ~/gmsd:new-project~ or update later with ~/gmsd:settings~. For the full config schema, workflow toggles, git branching options, and per-agent model breakdown, see the [[file:docs/USER-GUIDE.org][User Guide]].
...
*IMPORTANT:* GMSD includes built-in protections against committing secrets, but defense-in-depth is best practice. Deny read access to sensitive files as a first line of defense.
