---
name: gmsd-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /gmsd:plan-phase orchestrator.
...
**The difference:**
- `gmsd-verifier`: Verifies code DID achieve goal (after execution)
- `gmsd-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
```bash
INIT=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs init phase-op "${PHASE_ARG}")
```
...
Extract must_haves from each plan using gmsd-tools:

```bash
MUST_HAVES=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs frontmatter get "$PLAN_PATH" --field must_haves)
```
...
Use gmsd-tools plan-structure verification (already run in Step 2):

```bash
PLAN_STRUCTURE=$(node ~/.claude/get-my-shit-done/bin/gmsd-tools.cjs verify plan-structure "$PLAN_PATH")
```
...
Plans verified. Run `/gmsd:execute-phase {phase}` to proceed.
```
...
**DO NOT** check code existence — that's gmsd-verifier's job. You verify plans, not codebase.
